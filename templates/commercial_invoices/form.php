...
$defaultCompany = $dbInst->query("SELECT * FROM company WHERE status = 1 ORDER BY id ASC LIMIT 1")->fetch(\PDO::FETCH_ASSOC) ?: [];
?>
<div class="page-header d-flex justify-content-between align-items-center mb-4">
    <h1 class="h2 text-primary font-weight-bold mb-0"><i class="fas fa-file-invoice-dollar text-primary me-2"></i><?= escapeHtml($title) ?></h1>
    <a href="/commercial-invoices" class="btn btn-outline-secondary">Back to List</a>
</div>

<div class="card shadow-sm border-0">
    <div class="card-body p-4">
        <form method="post" action="<?= escapeHtml($action) ?>" class="needs-validation" id="ciForm" novalidate>
            <?= csrfToken() ?>
            ...
            <div class="bg-light p-3 rounded mb-4">
                ...
                <div class="row">
                    ...
                    <div class="col-md-3 mb-3">
                        <label class="form-label font-weight-bold" for="document_date">Invoice Date</label>
            ...
            <!-- Segment 2: Commercial details, Bank details, PO -->
            <div class="bg-light p-3 rounded mb-4">
                ...
                <div class="row">
                    ...
                    <div class="col-md-6 mb-3">
                        ...
                        <textarea id="notify_party" name="notify_party" class="form-control" rows="3"><?= escapeHtml($meta['notify_party'] ?? '') ?></textarea>
                    </div>
                </div>
            </div>

            <!-- Segment 3: Statutory Export Parameters -->
            <div class="bg-light p-3 rounded mb-4 border border-warning">
                <h5 class="text-warning font-weight-bold mb-3 border-bottom pb-2"><i class="fas fa-gavel me-2"></i>3. Statutory Export Details (Customs / Taxation)</h5>
                <div class="row">
                    <div class="col-md-4 mb-3">
                        <label class="form-label font-weight-bold" for="exporter_gst">Exporter GSTIN</label>
                        <input type="text" id="exporter_gst" name="exporter_gst" class="form-control" value="<?= escapeHtml($meta['exporter_gst'] ?? $defaultCompany['gst_number'] ?? '') ?>" required>
                    </div>
                    <div class="col-md-4 mb-3">
                        <label class="form-label font-weight-bold" for="lut_number">LUT (Letter of Undertaking) No</label>
                        <input type="text" id="lut_number" name="lut_number" class="form-control" value="<?= escapeHtml($meta['lut_number'] ?? $defaultCompany['lut_number'] ?? '') ?>">
                    </div>
                    <div class="col-md-4 mb-3">
                        <label class="form-label font-weight-bold" for="iec">Import Export Code (IEC)</label>
                        <input type="text" id="iec" name="iec" class="form-control" value="<?= escapeHtml($meta['iec'] ?? $defaultCompany['iec_code'] ?? '') ?>" required>
                    </div>
                </div>

                <div class="row">
                    <div class="col-md-4 mb-3">
                        <label class="form-label font-weight-bold" for="pan">Permanent Account Number (PAN)</label>
                        <input type="text" id="pan" name="pan" class="form-control" value="<?= escapeHtml($meta['pan'] ?? $defaultCompany['pan_number'] ?? '') ?>" required>
                    </div>
                    <div class="col-md-4 mb-3">
                        <label class="form-label font-weight-bold" for="ad_code">Authorized Dealer (AD) Code</label>
                        <input type="text" id="ad_code" name="ad_code" class="form-control" value="<?= escapeHtml($meta['ad_code'] ?? $defaultCompany['ad_code'] ?? '') ?>" required>
                    </div>
                    <div class="col-md-4 mb-3">
                        <label class="form-label" for="tax_basis">Tax Calculation Basis</label>
                        <select id="tax_basis" name="tax_basis" class="form-select">
                            <option value="lut" <?= ($invoice['tax_basis'] ?? 'lut') === 'lut' ? 'selected' : '' ?>>Zero-Rated Export (LUT 0% GST)</option>
                            <option value="igst_paid" <?= ($invoice['tax_basis'] ?? '') === 'igst_paid' ? 'selected' : '' ?>>Export IGST Refund Claim (Paid)</option>
                            <option value="domestic" <?= ($invoice['tax_basis'] ?? '') === 'domestic' ? 'selected' : '' ?>>Domestic Invoice (CGST/SGST/IGST)</option>
                        </select>
                    </div>
                </div>
                ...
                <div class="row border-top pt-3 mt-2">
                    <div class="col-md-6 mb-3">
                        <label class="form-label" for="shipping_bill_number">Shipping Bill Number (Post-customs)</label>
                        <input type="text" id="shipping_bill_number" name="shipping_bill_number" class="form-control" value="<?= escapeHtml($meta['shipping_bill_number'] ?? '') ?>">
                    </div>
                    <div class="col-md-6 mb-3">
                        <label class="form-label" for="shipping_bill_date">Shipping Bill Date</label>
                        <input type="date" id="shipping_bill_date" name="shipping_bill_date" class="form-control" value="<?= escapeHtml($meta['shipping_bill_date'] ?? '') ?>">
                    </div>
                </div>

                <div class="row">
                    <div class="col-md-6 mb-3">
                        <label class="form-label" for="export_declaration">Exporter General Declaration</label>
                        <textarea id="export_declaration" name="export_declaration" class="form-control" rows="2"><?= escapeHtml($meta['export_declaration'] ?? $defaultCompany['declaration'] ?? 'We declare that this invoice shows the actual price of the goods described and that all particulars are true and correct.') ?></textarea>
                    </div>
                    <div class="col-md-6 mb-3">
                        <label class="form-label" for="tax_declaration">Tax Regulation Declaration</label>
                        <textarea id="tax_declaration" name="tax_declaration" class="form-control" rows="2"><?= escapeHtml($meta['tax_declaration'] ?? 'We hereby declare that zero-rated supply of goods has been made under Letter of Undertaking (LUT) without payment of integrated tax.') ?></textarea>
                    </div>
                </div>
            </div>

            <!-- Segment 4: Exporter Bank Account Dropdown -->
            <div class="bg-light p-3 rounded mb-4">
                <h5 class="text-info font-weight-bold mb-3 border-bottom pb-2"><i class="fas fa-university me-2"></i>4. Exporter Banking Accounts</h5>
                <div class="row">
                    <div class="col-md-4 mb-3">
                        <label class="form-label font-weight-bold text-dark" for="company_bank_id">Select Company Bank</label>
                    ...
                    <div class="col-md-4 mb-3">
                        ...
                        <input type="text" id="beneficiary_name" name="beneficiary_name" class="form-control" value="<?= escapeHtml($meta['beneficiary_name'] ?? '') ?>">
                    </div>
                    <div class="col-md-4 mb-3">
                        <label class="form-label" for="bank_name">Bank Name</label>
                        <input type="text" id="bank_name" name="bank_name" class="form-control" value="<?= escapeHtml($meta['bank_name'] ?? '') ?>">
                    </div>
                </div>
                ...
                <div class="row">
                    <div class="col-md-4 mb-3">
                        ...
                        <input type="text" id="account_number" name="account_number" class="form-control" value="<?= escapeHtml($meta['account_number'] ?? '') ?>">
                    </div>
                    <div class="col-md-4 mb-3">
                        <label class="form-label" for="swift_code">SWIFT Code</label>
                        <input type="text" id="swift_code" name="swift_code" class="form-control" value="<?= escapeHtml($meta['swift_code'] ?? '') ?>">
                    </div>
                    <div class="col-md-4 mb-3">
            ...
            <div class="d-flex justify-content-end gap-2 border-top pt-4">
                <a href="/commercial-invoices" class="btn btn-outline-secondary">Cancel</a>
                <button type="submit" class="btn btn-success"><i class="fas fa-save me-1"></i> Save Commercial Invoice</button>
            </div>
        </form>
    </div>
</div>
