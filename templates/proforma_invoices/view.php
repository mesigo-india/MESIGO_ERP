...
<div class="page-header no-print">
    ...
    <div class="btn-group">
        <a href="/proforma-invoices" class="btn btn-outline-secondary">Back</a>
        <a href="/proforma-invoices/<?= (int) ($quotation['id'] ?? 0) ?>/edit" class="btn btn-outline-primary">Edit</a>
        <button onclick="window.print()" class="btn btn-outline-dark">Print</button>
        <form method="post" action="/proforma-invoices/<?= (int) ($quotation['id'] ?? 0) ?>/email" class="d-inline"><?= csrfToken() ?><button class="btn btn-outline-info">Email Ready</button></form>
        <form method="post" action="/proforma-invoices/<?= (int) ($quotation['id'] ?? 0) ?>/convert" class="d-inline" onsubmit="return confirm('Convert Proforma Invoice to Commercial Invoice?');"><?= csrfToken() ?><button class="btn btn-success">Convert to CI</button></form>
    </div>
</div>

<?php $totals = $meta['totals'] ?? []; $statusClasses = [0 => 'bg-secondary', 2 => 'bg-success', 4 => 'bg-info', 5 => 'bg-primary', 6 => 'bg-danger']; ?>
<div class="card">
    <div class="card-body">
        ...
        <div class="row mb-3">
            <div class="col-md-6">
                <strong>PI No:</strong> <?= escapeHtml($quotation['document_number'] ?? '') ?><br>
        ...
        <div class="row mb-3">
            ...
            <div class="col-md-6"><strong>Destination Port:</strong> <?= escapeHtml($quotation['delivery_port_name'] ?? '') ?></div>
            <div class="col-md-6"><strong>Payment Terms:</strong> <?= escapeHtml($quotation['payment_term_name'] ?? '') ?></div>
            <div class="col-md-6"><strong>Shipment Terms:</strong> <?= escapeHtml($meta['shipment_term'] ?? ($quotation['shipment_type'] ?? '')) ?></div>
        </div>
        <div class="table-responsive">
            <table class="table table-bordered">
                ...
                <tbody><?php foreach ($items as $index => $item): ?><tr><td><?= $index + 1 ?></td><td><?= escapeHtml($item['product_name'] ?? '') ?></td><td><?= escapeHtml($item['hsn_code'] ?? '') ?></td><td><?= escapeHtml($item['packing_type_name'] ?? '') ?></td><td class="text-end"><?= number_format((float) ($item['quantity'] ?? 0), 3) ?></td><td><?= escapeHtml($item['unit_code'] ?? '') ?></td><td class="text-end"><?= number_format((float) ($item['rate'] ?? 0), 4) ?></td><td class="text-end"><?= number_format((float) ($item['discount_amount'] ?? 0), 2) ?></td><td class="text-end"><?= number_format((float) ($item['tax_amount'] ?? 0), 2) ?></td><td class="text-end"><?= number_format((float) ($item['net_amount'] ?? 0), 2) ?></td></tr><?php endforeach; ?></tbody>
            </table>
        </div>
        <div class="row">
...
<div class="row mt-3 no-print">
    <div class="col-md-6"><div class="card"><div class="card-body"><h5>Revision History</h5><?php foreach ($revisions as $revision): ?><div>Rev <?= (int) $revision['revision_number'] ?> - <?= escapeHtml($revision['created_at']) ?> - <?= escapeHtml($revision['revision_notes'] ?? '') ?></div><?php endforeach; ?><form method="post" action="/proforma-invoices/<?= (int) ($quotation['id'] ?? 0) ?>/revise" class="mt-2"><?= csrfToken() ?><input type="text" name="revision_notes" class="form-control mb-2" placeholder="Revision notes"><button class="btn btn-sm btn-primary">Save Revision</button></form></div></div></div>
    <div class="col-md-6"><div class="card"><div class="card-body"><h5>Status Workflow</h5><form method="post" action="/proforma-invoices/<?= (int) ($quotation['id'] ?? 0) ?>/status" class="row g-2"><?= csrfToken() ?><div class="col-md-5"><select name="status" class="form-select"><?php foreach ($statuses as $statusId => $statusName): ?><option value="<?= (int) $statusId ?>" <?= (int) ($quotation['status'] ?? 0) === (int) $statusId ? 'selected' : '' ?>><?= escapeHtml($statusName) ?></option><?php endforeach; ?></select></div><div class="col-md-5"><input type="text" name="remarks" class="form-control" placeholder="Status remarks"></div><div class="col-md-2"><button class="btn btn-primary">Update</button></div></form><?php foreach ($history as $entry): ?><div class="small text-muted mt-1"><?= escapeHtml($entry['created_at']) ?>: <?= escapeHtml($statuses[(int) $entry['new_status']] ?? 'Unknown') ?> <?= escapeHtml($entry['remarks'] ?? '') ?></div><?php endforeach; ?></div></div></div>
</div>