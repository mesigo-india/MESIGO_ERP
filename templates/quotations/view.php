...
<div class="page-header no-print d-flex align-items-center justify-content-between mb-4">
    <div>
        <h4 class="mb-0 fw-bold"><i class="fas fa-file-invoice-dollar text-primary me-2"></i>Quotation <?= escapeHtml($quotation['document_number']) ?> <span class="badge <?= $statusClasses[(int)($quotation['status'] ?? 0)] ?> fs-6 ms-2"><?= \App\Core\DocumentStatusEngine::getStatusLabel((int)($quotation['status'] ?? 0)) ?></span></h4>
    ...
    <div class="d-flex gap-2 align-items-center">
        ...
        <div class="dropdown d-inline-block">
            ...
            <ul class="dropdown-menu dropdown-menu-end shadow border-0 mt-1">
                <li><a href="/quotations/<?= (int) $quotation['id'] ?>/print?mode=plain" target="_blank" class="dropdown-item"><i class="fas fa-file-alt me-2 text-muted"></i> Plain Paper Mode</a></li>
                <?php if (!empty($company['letterhead_path'])): ?>
                    <li><a href="/quotations/<?= (int) $quotation['id'] ?>/print?mode=letterhead&src=<?= urlencode('/uploads/' . $company['letterhead_path']) ?>" target="_blank" class="dropdown-item"><i class="fas fa-file-image me-2 text-muted"></i> Default Letterhead</a></li>
                <?php endif; ?>
                <?php if (!empty($company['letterhead_export_path'])): ?>
                    <li><a href="/quotations/<?= (int) $quotation['id'] ?>/print?mode=letterhead&src=<?= urlencode('/uploads/' . $company['letterhead_export_path']) ?>" target="_blank" class="dropdown-item"><i class="fas fa-ship me-2 text-muted"></i> Export Letterhead</a></li>
                <?php endif; ?>
                <?php if (!empty($company['letterhead_domestic_path'])): ?>
                    <li><a href="/quotations/<?= (int) $quotation['id'] ?>/print?mode=letterhead&src=<?= urlencode('/uploads/' . $company['letterhead_domestic_path']) ?>" target="_blank" class="dropdown-item"><i class="fas fa-building me-2 text-muted"></i> Domestic Letterhead</a></li>
                <?php endif; ?>
            </ul>
        </div>
        ...
        <?php if ((int)($quotation['status'] ?? 0) === 0 && !$downstream): ?>
            <form method="post" action="/quotations/<?= (int) $quotation['id'] ?>/delete" class="d-inline" onsubmit="const reason = prompt('Please enter a reason for deletion (optional):'); if (reason === null) return false; this.querySelector('input[name=delete_reason]').value = reason; return confirm('Are you sure you want to delete this Quotation?');">
                <?= csrfToken() ?>
...
<?php if ($isLocked): ?>
    <div class="alert alert-warning d-flex align-items-center mb-4 no-print" role="alert">
        ...
        <div>
            ...
            <?php if ($downstream): ?>
                This Quotation is locked because downstream <?= htmlspecialchars($downstream['type_name']) ?> (<a href="/<?= htmlspecialchars($downstream['type_code']) ?>s/<?= $downstream['id'] ?>"><?= htmlspecialchars($downstream['document_number']) ?></a>) has been created.
            <?php else: ?>
                This Quotation is locked under status <strong><?= \App\Core\DocumentStatusEngine::getStatusLabel((int)$quotation['status']) ?></strong>.
    ...
    <div class="col-md-6 mb-3">
        <div class="card border-0 shadow-sm">
            <div class="card-body">
                ...
                <?php if ((int)$quotation['status'] === 2): ?>
                    <form method="post" action="/quotations/<?= (int) $quotation['id'] ?>/revise" class="mt-2">
                        <?= csrfToken() ?>
        ...
        <div class="card border-0 shadow-sm">
            <div class="card-body">
                ...
                <?php if (count($allowedTransitions) > 1): ?>
                    <form method="post" action="/quotations/<?= (int) $quotation['id'] ?>/status" class="row g-2">
                        <?= csrfToken() ?>
                        <div class="col-md-5">
